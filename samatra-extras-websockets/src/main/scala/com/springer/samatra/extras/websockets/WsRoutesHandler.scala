...
trait WsRoutesHandler {
  ...
  def addWsRoutes(pathSpec: String, wSControllers: WSController*): this.type = {
    ...
    WsRoutes(initialize(self), pathSpec, wSControllers: _*)

    //add routes
    wSControllers.foreach { wc =>
      self.routesWithContextBuffer.append(spec ->
        //hack for printing WsRoutes when they aren't really routes
        new WebSocketRoutes(wc.routes.toSeq.map(r => PathParamsRoute(GET, r.path, r.socket.asInstanceOf[Function[Request, HttpResp]])), wc.getClass))
    }
